...
from . import p, zero_address, EURS, FAUCET
import logging
...
class Markets():
    ...
    def new(self, nameMarket, nameAccount, fee=0, connection1= zero_address, connection2=zero_address):
        ...
        market = self.marketContract.deploy( EURS, EnergyToken, connection1, connection2,  Registry, fee, {"from": account.address})
...
class Market():
    def __init__(self, market, account, name):
        self.market = market
        EURS.transfer(self.market.address, 1000, {"from":  FAUCET})
        self.owner = account
        self.name = name
